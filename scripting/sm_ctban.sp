/*
  ...
  1.6.1 Support for new SM1.4 natives, Added confi file generation
...
{
  name = "CT Ban",
  author = "databomb, modified by Invex | Byte",
...
public OnPluginStart()
{
  ...
  RegAdminCmd("sm_ctban_remoteupdate", Command_CTBan_RemoteUpdate, ADMFLAG_ROOT, "sm_ctban_remoteupdate <steamid> - Allows CT ban information to be updated to match database.");
...
//Allows bans to be changed/edited from the website panel
...
public DB_Callback_OnClientAuthed(Handle:owner, Handle:hndl, const String:error[], any:client)
{
  ...
  {
    LogError("Error in OnClientAuthorized or RemoteUpdate query: %s", error);
  }
  else
  {
    ...
    if (iRowCount)
    {
      ...
      new iBanTime = SQL_FetchInt(hndl, 1);
      
      if (iBanTime == 0) {
        new iBannedArrayIndex = FindValueInArray(gA_TimedBanLocalList, client);
        //If entry does exits, remove it
        if (iBannedArrayIndex != -1)
          RemoveFromArray(gA_TimedBanLocalList, client);
      ...
      //Else ensure ban cookie is set to false
      else
        SetClientCookie(client, g_CT_Cookie, "0");
        
      //Handles team swapping if banned
...
public CP_Callback_CheckBan(Handle:owner, Handle:hndl, const String:error[], any:stringPack)
{
  ...
  else
  {
    ...
    decl String:temp[300];
    new String:escapedPerpName[300];
    Format(temp, sizeof(temp), "%s", sTargetName);
    SQL_EscapeString(gH_BanDatabase, temp, escapedPerpName, sizeof(escapedPerpName));
    Format(logQuery, sizeof(logQuery), "INSERT INTO %s (timestamp, perp_steamid, perp_name, admin_steamid, admin_name, bantime, timeleft, reason) VALUES (%d, '%s', '%s', '%s', 'Console', 0, 0, 'Rage ban')", g_sLogTableName, iTimeStamp, authID, escapedPerpName, adminSteamID);
    #if DEBUG == 1
...
//Update local variables to match database
public Action:Command_CTBan_RemoteUpdate(client, args)
{
  if (!args)
    return Plugin_Handled;
    
  decl String:sProvidedSteamId[32];
  GetCmdArgString(sProvidedSteamId, sizeof(sProvidedSteamId));
...
{
	decl String:sDisconnectSteamID[32];
...
PerformCTBan(client, adminclient, banTime=0, reason=0, String:manualReason[]="")
{
  ...
  if(adminclient && IsClientInGame(adminclient))
  {
    ...
    decl String:temp[300];
    new String:escapedPerpName[300];
    new String:escapedAdminName[300];
    new String:escapedReason[300];
    Format(temp, sizeof(temp), "%N", client);
    SQL_EscapeString(gH_BanDatabase, temp, escapedPerpName, sizeof(escapedPerpName));
    Format(temp, sizeof(temp), "%N", adminclient);
    SQL_EscapeString(gH_BanDatabase, temp, escapedAdminName, sizeof(escapedAdminName));
    Format(temp, sizeof(temp), "%s", sReason);
    SQL_EscapeString(gH_BanDatabase, temp, escapedReason, sizeof(escapedReason));
    Format(logQuery, sizeof(logQuery), "INSERT INTO %s (timestamp, perp_steamid, perp_name, admin_steamid, admin_name, bantime, timeleft, reason) VALUES (%d, '%s', '%s', '%s', '%s', %d, %d, '%s')", g_sLogTableName, timestamp, targetSteam, escapedPerpName, adminSteam, escapedAdminName, banTime, banTime, escapedReason);
    #if DEBUG == 1
    LogMessage("log query: %s", logQuery);
    #endif
  ...
  else
  {
    ...
    Format(temp, sizeof(temp), "%s", sReason);
    SQL_EscapeString(gH_BanDatabase, temp, escapedReason, sizeof(escapedReason));
    Format(logQuery, sizeof(logQuery), "INSERT INTO %s (timestamp, perp_steamid, perp_name, admin_steamid, admin_name, bantime, reason) VALUES (%d, '%s', '%s', 'STEAM_0:1:1', 'Console', %d, %d, '%s')", g_sLogTableName, timestamp, targetSteam, escapedPerpName, banTime, banTime, escapedReason);
    #if DEBUG == 1
    LogMessage("log query: %s", logQuery);
    #endif
    SQL_TQuery(gH_BanDatabase, DB_Callback_CTBan, logQuery, client);
